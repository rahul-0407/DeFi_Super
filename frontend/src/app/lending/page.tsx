...
const TABS = [
  { key: "supply" as const, label: "Supply Assets", color: "bg-[#203f34]" },
  { key: "pool" as const, label: "Liquidity", color: "bg-[#2d5244]" },
  {
    key: "borrow" as const,
    label: "Borrow",
    color: "bg-[#77b8a2] text-black",
  },
];

export default function LendingPage() {
  const { address, isConnected } = useAccount();
  const [activeTab, setActiveTab] = useState<"supply" | "pool" | "borrow">(
    "supply",
  );
  ...
  const { data: usdcBalance, refetch: refetchUsdcBal } = useBalance({
    address,
    token: usdc,
  });
  const { data: dUsdcBalance, refetch: refetchDUsdcBal } = useBalance({
    address,
    token: CONTRACT_ADDRESSES.D_USDC,
  });
  ...
  const calculateRates = () => {
    if (!totalBorrowedPool || !poolBalance)
      return { borrowAPR: 4.2, supplyAPY: 2.1 };
    ...
    if (totalLiquidity === 0)
      return {
        borrowAPR: Number(formatUnits((baseRate as bigint) || 0n, 18)) * 100,
        supplyAPY: 0,
      };
  ...
  const handleAction = async (
    ...
    if (action === "deposit") {
      if (parsedAmount > (ethBalance?.value || 0n)) {
        setError(
          `Insufficient WETH balance. You have ${ethBalance?.formatted} WETH.`,
        );
        return;
      }
    }
    if (action === "withdraw") {
      if (parsedAmount > collateral) {
        setError(
          `Insufficient collateral supplied. You only have ${formatUnits(collateral, 18)} WETH.`,
        );
        return;
      }
    }
    ...
    if (action === "withdrawLiquidity") {
      if (parsedAmount > ((userLiquidity as bigint) || 0n)) {
        setError(
          `Insufficient liquidity supplied. You only have ${formatUnits((userLiquidity as bigint) || 0n, 6)} USDC.`,
        );
        return;
      }
    }
  ...
  const displayError =
    error ||
    (wagmiError
      ? (wagmiError as any).shortMessage || wagmiError.message
      : null);

  const headerStats = [
    {
      label: "Collateral",
      value: `${formatUnits(collateral, 18).slice(0, 8)} WETH`,
      color: "text-blue-400",
      border: "border-blue-500/10",
    },
    {
      label: "Borrowed",
      value: `${formatUnits((borrowBalance as bigint) || 0n, 6).slice(0, 8)} USDC`,
      color: "text-red-400",
    ...
    },
    {
      label: "Health Factor",
      value: hfFormatted,
      color: Number(hfFormatted) < 1.1 ? "text-red-400" : "text-green-400",
      border:
        Number(hfFormatted) < 1.1 ? "border-red-500/10" : "border-green-500/10",
    },
  ];
      ...
      >
        <div>
          <h2 className="text-2xl font-serif font-bold tracking-tight">
            Pareto Lending Markets
          </h2>
          <p className="text-gray-500 text-sm mt-1">
            Supply liquidity to earn interest or draw credit against your
            collateral in a secure, decentralized environment.
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          {headerStats.map((s) => (
            <div
              key={s.label}
              className={`glass px-4 py-2.5 rounded-xl flex items-center gap-2 border ${s.border}`}
            >
          ...
          <div className="glass rounded-2xl p-6 space-y-6">
            ...
            <div className="bg-white/[0.04] rounded-2xl p-5 border border-white/[0.06] focus-within:border-white/[0.15] transition-all">
              <div className="flex justify-between text-[11px] text-gray-500 mb-4 uppercase tracking-wider">
                ...
                <span
                  className="cursor-pointer hover:text-white transition-colors normal-case"
                  onClick={() => {
                    if (activeTab === "supply")
                      setAmount(ethBalance?.formatted || "0");
                    else if (activeTab === "pool")
                      setAmount(usdcBalance?.formatted || "0");
                    else
                      setAmount(
                        formatUnits((borrowBalance as bigint) || 0n, 6),
                      );
                  }}
                >
                  {activeTab === "supply"
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 bg-white/[0.08] px-4 py-2 rounded-xl font-bold text-sm shrink-0">
                  <div
                    className={`w-2 h-2 rounded-full ${activeTab === "supply" ? "bg-deep-green" : "bg-secondary-fixed"}`}
                  />
              ...
              {activeTab === "supply" ? (
                <>
                  <button
                    ...
                    disabled={
                      ...
                      isTxLoading ||
                      !isConnected ||
                      parseUnits(amount, 18) > (ethBalance?.value || 0n)
                    }
                    className="py-3.5 bg-deep-green text-secondary-fixed hover:bg-[#2d5244] disabled:opacity-40 disabled:cursor-not-allowed rounded-xl font-bold text-sm shadow-lg shadow-black/20 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                  >
                    {isTxLoading ? (
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      isTxLoading ||
                      !isConnected ||
                      parseUnits(amount, 18) > collateral
                    }
                ...
                <div className="col-span-2 flex flex-col gap-3">
                  <button
                    ...
                    disabled={
                      ...
                      isTxLoading ||
                      !isConnected ||
                      parseUnits(amount, 6) > (usdcBalance?.value || 0n)
                    }
                    className="py-3.5 bg-[#2d5244] text-[#f0f4f0] hover:bg-[#3d6b5a] disabled:opacity-40 disabled:cursor-not-allowed rounded-xl font-bold text-sm shadow-lg shadow-black/20 transition-all active:scale-[0.98] flex items-center justify-center gap-2 w-full"
                  >
                    {isTxLoading ? (
                      <Loader2 size={16} className="animate-spin" />
                    ) : (
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      isTxLoading ||
                      !isConnected ||
                      parseUnits(amount, 6) > ((userLiquidity as bigint) || 0n)
                    }
                    className="py-3.5 bg-white/[0.04] hover:bg-white/[0.08] disabled:opacity-30 disabled:cursor-not-allowed rounded-xl font-bold text-sm border border-white/[0.08] transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                  >
                    Withdraw Liquidity
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-1">
                    <div className="flex justify-between items-center px-4 py-3 bg-white/[0.03] rounded-xl border border-white/[0.06]">
                      ...
                      <span className="text-xs font-bold text-indigo-400">
                        {dUsdcBalance?.formatted.slice(0, 8) || "0.00"}
                      </span>
                    </div>
                    <div className="flex justify-between items-center px-4 py-3 bg-white/[0.03] rounded-xl border border-white/[0.06]">
                      <span className="text-[10px] text-gray-500 uppercase">
                      ...
                      <span className="text-xs font-bold text-white">
                        {userLiquidity
                          ? formatUnits(userLiquidity as bigint, 6).slice(0, 8)
                          : "0.00"}{" "}
                        USDC
                      </span>
                    </div>
                ...
                <>
                  <button
                    ...
                    className="py-3.5 bg-secondary-fixed text-black hover:bg-[#c2f970] disabled:opacity-40 disabled:cursor-not-allowed rounded-xl font-bold text-sm shadow-lg shadow-secondary-fixed/20 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
                  >
                    {isTxLoading ? (
                      <Loader2 size={16} className="animate-spin" />
                    ) : (
        ...
        >
          <div className="glass p-6 rounded-2xl border border-blue-500/10">
            ...
            <div className="space-y-0">
              {[
                {
                  label: "Structured Yield (WETH)",
                  value: `${supplyAPY.toFixed(2)}%`,
                  color: "text-green-400",
                  icon: TrendingUp,
                },
                {
                  label: "Borrow Rate (USDC)",
                ...
                return (
                  <div
                    key={item.label}
                    className={`flex justify-between items-center py-3.5 ${idx < 3 ? "border-b border-white/[0.05]" : ""}`}
                  >
            ...
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-gray-500 mt-1 leading-relaxed">
                  Supply WETH as collateral to borrow USDC or earn yield. The
                  protocol manages risk and liquidation thresholds
                  automatically.
                </p>
              </div>
            </div>
